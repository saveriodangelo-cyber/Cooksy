...
class Bridge:
    def __init__(self, *args: Any, progress: Any = None, **kwargs: Any) -> None:
        ...
        try:
            import sys
            from pathlib import Path
            logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
            with open(logfile, "w", encoding="utf-8") as f:
                f.write("[BRIDGE] __init__ chiamato\n")
                f.write(f"[BRIDGE] sys.frozen = {getattr(sys, 'frozen', False)}\n")
                f.write(f"[BRIDGE] sys.executable = {sys.executable}\n")
        except Exception as e:
            print(f"Errore log init: {e}")
    ...
    def exit_app(self, *_args: Any, **_kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            log_event("error", f"exit_app failed: {e}")
            sys.exit(1)
    ...
    # ============ PASSKEY / WEBAUTHN ============
    # SECURITY: Feature disabilitata - WebAuthn senza verifica FIDO2 è vulnerabile

    def passkey_start_registration(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """DISABILITATO: WebAuthn non è sicuro senza verifica FIDO2 completa."""
        return {"ok": False, "error": "Autenticazione Passkey disabilitata per motivi di sicurezza"}
    ...
    def get_templates(self, *_args: Any, **_kwargs: Any) -> Dict[str, Any]:
        try:
            ...
            with open(logfile, "a", encoding="utf-8") as f:
                f.write("[BRIDGE] get_templates chiamato\n")
        except:
            pass
        ...
        try:
            ...
            if isinstance(t, list) and t:
                ...
                try:
                    logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
                    with open(logfile, "a", encoding="utf-8") as f:
                        f.write(f"[BRIDGE] Templates da pipeline: {len(templates)}\n")
                except:
                    pass
        except Exception as e:
            try:
                logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
                with open(logfile, "a", encoding="utf-8") as f:
                    f.write(f"[BRIDGE] Errore pipeline: {e}\n")
            except:
                pass
            try:
                ...
                try:
                    logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
                    with open(logfile, "a", encoding="utf-8") as f:
                        f.write(f"[BRIDGE] Errore fallback pdf_export: {e2}\n")
                except:
                    pass
        ...
        try:
            logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
            with open(logfile, "a", encoding="utf-8") as f:
                f.write(f"[BRIDGE] Elaboro {len(templates)} template raw\n")
        except:
            pass
        ...
        try:
            logfile = Path(sys.executable).parent / "bridge_init.txt" if getattr(sys, "frozen", False) else Path("bridge_init.txt")
            with open(logfile, "a", encoding="utf-8") as f:
                f.write(f"[BRIDGE] Ritorno {len(clean)} template clean: {clean[:5]}\n")
        except:
            pass
    ...
    def render_template_preview(self, payload: Any = None, *_args: Any, **_kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                example_html = pipeline.render_html_template(example_recipe, template_id, assets_dir=assets_dir) or ""
                
                # Pulisce i dati reali prima del rendering
                user_recipe_clean = pipeline.clean_recipe_data(user_recipe)
                result_html = pipeline.render_html_template(user_recipe_clean, template_id, assets_dir=assets_dir) or ""
                ...
                def _debug_block() -> str:
                    debug_payload = {
                        ...
                        "result_fields": sorted(list(_compact(user_recipe_clean).keys())),
                        "result": _compact(user_recipe_clean),
                    }
                    try:
...
<head>
    ...
    <style>
        ...
        html, body {{ margin: 0; padding: 0; background: var(--bg); color: var(--accent); font-family: 'Segoe UI', Tahoma, sans-serif; }}
        .layout {{ display: flex; flex-direction: column; gap: 12px; padding: 12px; min-height: 100vh; box-sizing: border-box; }}
        .toolbar {{ display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 8px; background: var(--panel); border: 1px solid rgba(255,255,255,0.06); border-radius: 10px; padding: 10px 12px; box-shadow: 0 12px 30px rgba(0,0,0,0.35); }}
        .tabs {{ display: flex; flex-wrap: wrap; gap: 6px; align-items: center; }}
        .tab {{ border: 1px solid rgba(255,255,255,0.18); background: rgba(255,255,255,0.04); color: var(--accent); padding: 6px 10px; border-radius: 8px; cursor: pointer; transition: all 0.15s ease; font-size: 13px; letter-spacing: 0.2px; }}
        .tab:hover {{ border-color: rgba(255,255,255,0.32); background: rgba(255,255,255,0.08); }}
        .tab.is-active {{ background: #e1ecff; color: #0c1624; border-color: #e1ecff; box-shadow: 0 8px 24px rgba(0,0,0,0.25); }}
        .subtitle {{ color: var(--muted); font-size: 13px; letter-spacing: 0.2px; }}
        .debugToggle {{ display: inline-flex; align-items: center; gap: 6px; color: var(--muted); font-size: 13px; padding: 6px 8px; border-radius: 8px; background: rgba(255,255,255,0.05); border: 1px dashed rgba(255,255,255,0.14); }}
        .views {{ display: grid; grid-template-columns: 1fr; gap: 12px; }}
        .view {{ display: none; background: var(--card); border-radius: 12px; border: 1px solid var(--border); box-shadow: 0 14px 42px rgba(0,0,0,0.28); overflow: hidden; }}
        .view.is-active {{ display: block; }}
        .iframeWrap {{ width: 100%; height: calc(90vh - 120px); background: #fff; }}
        iframe.preview {{ width: 100%; height: 100%; border: none; background: #fff; }}
        .debugBox {{ padding: 14px; background: #0c111a; color: #c7d4e5; font-family: 'Consolas','SFMono-Regular','Menlo',monospace; font-size: 12px; overflow: auto; min-height: 200px; box-sizing: border-box; }}
        .badge {{ display: inline-flex; align-items: center; gap: 6px; padding: 4px 10px; background: rgba(255,255,255,0.06); border-radius: 8px; color: var(--muted); font-size: 12px; letter-spacing: 0.3px; }}
    </style>
</head>
<body>
    <div class=\"layout\">
        <div class=\"toolbar\">
            <div class=\"tabs\">
                <button class=\"tab is-active\" data-target=\"example\">Esempio completo</button>
                <button class=\"tab\" data-target=\"result\">Risultato (dati inseriti)</button>
                <!-- Debug disabilitato in produzione -->
            </div>
            <div class=\"subtitle\">Template: {html_utils.escape(template_id)} • Anteprima pulita (senza placeholder)</div>
        </div>

        <div class=\"views\">
            <div class=\"view is-active\" data-view=\"example\">
                <div class=\"badge\">Esempio dimostrativo</div>
                <div class=\"iframeWrap\"><iframe class=\"preview\" srcdoc=\"{_srcdoc(example_html)}\"></iframe></div>
            </div>
            <div class=\"view\" data-view=\"result\">
                <div class=\"badge\">Solo campi presenti nella ricetta</div>
                <div class=\"iframeWrap\"><iframe class=\"preview\" srcdoc=\"{_srcdoc(result_html)}\"></iframe></div>
            </div>
            <div class=\"view\" data-view=\"debug\" id=\"debugView\" style=\"display:none\">
                <div class=\"debugBox\"><pre style=\"margin:0; white-space:pre-wrap;\">{_debug_block()}</pre></div>
            </div>
        </div>
    </div>
    <script>
        (function() {{
            ...
            function showView(id) {{
                views.forEach(v => v.classList.toggle('is-active', v.getAttribute('data-view') === id));
                tabs.forEach(t => t.classList.toggle('is-active', t.getAttribute('data-target') === id));
            }}
            tabs.forEach(t => t.addEventListener('click', () => showView(t.getAttribute('data-target'))));
            if (dbg) {{
                dbg.addEventListener('change', () => {{
                    const v = document.querySelector('[data-view="debug"]');
                    if (!v) return;
                    v.style.display = dbg.checked ? 'block' : 'none';
                    if (dbg.checked) showView('debug'); else showView('result');
                }});
            }}
        }})();
    </script>
                ...
                # Errore di rendering: ritorna messaggio esplicito
...
<head>
  <meta charset="utf-8" />
  <style>
    body {{ font-family: 'Segoe UI', sans-serif; background: #0f1622; color: #e2f4ff; padding: 20px; }}
    .error {{ background: #2a1a1a; border: 1px solid #d32f2f; border-radius: 8px; padding: 16px; }}
    .error h2 {{ color: #ff6b6b; margin-top: 0; }}
    .error code {{ background: #0c111a; color: #c7d4e5; padding: 8px; border-radius: 4px; display: block; margin: 12px 0; overflow-x: auto; font-size: 12px; }}
  </style>
</head>
<body>
  <div class="error">
    <h2>Errore rendering template</h2>
    <p>Il template non può essere renderizzato. Possibili cause:</p>
    <ul>
      <li>Variabile non definita o mancante</li>
      <li>Include non trovato (es. _demo_recipe.html)</li>
      <li>Sintassi Jinja2 errata nel template</li>
      <li>Tag Jinja non renderizzati (variabili sconosciute)</li>
    </ul>
    <code>{html_utils.escape(error_msg)}</code>
  </div>
</body>
</html>"""
                return {"ok": False, "error": error_msg, "html": error_html}
        except Exception as e:
            error_msg = str(e)
        ...
        try:
            ...
            return {
                ...
                "features": {
                    ...
                    "ai_priority": features.ai_priority,
    ...
    def get_available_templates(self, *_args: Any, **_kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            templates_list_path = project_root() / "templates" / "templates_list.json"
            all_templates = []
            ...
            available = get_available_templates(tier, all_templates or ["classico"])
                ...
                with self._analysis_lock:
                    ...
                    try:
                        if not self._analysis_usage_recorded and isinstance(res, dict) and res.get("ok") and isinstance(res.get("recipe"), dict):
                            self._record_ai_usage(res.get("recipe"))
...
# ------------------------------------------------------------
    ...
    def get_legal_document(self, doc_type: str = "terms", *_args: Any, **_kwargs: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            log_event("error", f"get_legal_document failed: {e}")
            return {"ok": False, "error": str(e)}