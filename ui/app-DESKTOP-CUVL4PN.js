(function () {
  ...
  function setTplZoom(val) {
    tplZoom = clamp(Number(val || 1), 0.5, 2.5);
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  function formatKeyHint(info) {
    if (!info || !info.has_key) return 'Key assente';
  ...
  async function testAiSettings() {
    ...
    try {
      ...
      if (!res || !res.ok) {
        setAiStatus(`Test fallito: ${res?.error || 'sconosciuto'}`);
        return;
      }
  ...
  async function authPasskeyRegister() {
    ...
    try {
      ...
      const start = await api('passkey_start_registration', { email });
      if (!start || !start.ok) {
      ...
      const userId = Uint8Array.from(atob(start.user.id), c => c.charCodeAt(0));
  ...
  function renderArchiveRows(items) {
    ...
    rows.forEach((it) => {
      ...
      td2.textContent = it.title || ''; // XSS-safe

      const td3 = document.createElement('td');
      td3.textContent = it.category || ''; // XSS-safe

      const td4 = document.createElement('td');
      td4.textContent = formatDateIt(it.updated_at || '');
  ...
  async function loadTemplates() {
    console.log('[loadTemplates] INIZIO - funzione chiamata');
    
    // Aspetta che l'API sia pronta (max 10 secondi)
    let retries = 0;
    while (!apiReady() && retries < 50) {
      console.log(`[loadTemplates] API non ancora pronta, attendo... (tentativo ${retries+1}/50)`);
      await new Promise(r => setTimeout(r, 200));
      retries++;
    }
    
    if (!apiReady()) {
      console.error('[loadTemplates] API non pronta dopo 10 secondi, uso fallback Default');
    }
    ...
    try {
      ...
      if (!api) {
        console.log('[loadTemplates] API non disponibile, uso fallback');
        log('API non pronta per loadTemplates, uso fallback Default');
        res = null; // prosegui al fallback
      } else {
        console.log('[loadTemplates] API disponibile, chiamo get_templates');
      }
      if (api && typeof api.get_templates === 'function') {
        console.log('[loadTemplates] Chiamata a api.get_templates()...');
        res = await api.get_templates();
        console.log('[loadTemplates] Risposta ricevuta:', res);
        log(`Template caricati: ${res ? (Array.isArray(res) ? res.length : res.templates?.length || 0) : 0}`);
      } else if (typeof api.list_templates === 'function') {
        ...
        log('Nessun metodo template disponibile (get_templates o list_templates). Fallback Default.');
        res = null; // prosegui al fallback
      }
    } catch (e) {
      log(`Errore caricamento template: ${e.message || e}. Fallback Default.`);
      res = null; // prosegui al fallback
    }
    ...
    if (!clean.length) clean.push({ id: 'html:Template_Ricetta_AI', label: 'Default' });

    console.log(`[loadTemplates] Popolo selettore con ${clean.length} template:`, clean.map(t=>t.label||t.id));
    ...
    if (selTemplate) {
      ...
      console.log(`[loadTemplates] Selettore popolato con ${selTemplate.options.length} opzioni. Valore selezionato: ${selTemplate.value}`);
    } else {
      console.error('[loadTemplates] selTemplate NON TROVATO nel DOM!');
    }
  ...
  const container = el('termsFullContent');
  if (container) {
  ...
  // Termini rimossi dall'app - nessuna azione necessaria
}
  }

// Funzione rimossa: la richiesta di accettazione Termini e Condizioni all'avvio è stata eliminata su richiesta.
...
function initWhenReady() {
  console.log('[initWhenReady] Controllo API ready:', apiReady());
  if (!apiReady()) return;
  init();
}
...
// Fallback: se l'API non è ready ma pywebviewready non scatta, prova dopo un po'
setTimeout(() => {
  console.log('[Fallback] Controllo API ready dopo 2 secondi:', apiReady());
  if (apiReady()) {
    initWhenReady();
  }
}, 2000);
